...
# Since year4 is integer, we can use the number (1999).
Ex1 <- filter(NTL.phys.data, year4 == 1999)
Ex1_otherway <- NTL.phys.data %>% 
  filter(year4 == 1999)
  
# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
# Below is my practice
#NTL.phys.data.ex2 <- filter(NTL.phys.data, lakename == "Tuesday Lake")
#NTL.phys.data.ex3 <- filter(NTL.phys.data.ex2, year4 %>% c("1990", "1994"))

Ex2 <- filter(NTL.phys.data, year4 %in% c(1990:1999) & lakename == "Tuesday Lake")
Ex2_otherway <- NTL.phys.data %>%
  filter(year4 %in% c(1990:1999), lakename == "Tuesday Lake")
...
Ex3 <- NTL.phys.data %>% 
  ...
  mutate(month = month(sampledate) ) %>%  # Use the lubridate
  filter(month == 7)

Ex3_otherway <- NTL.phys.data %>% 
  filter(lakename == "Tuesday Lake", year4 %in% c(1990:1999)) %>% 
  filter(month(sampledate)== 7)   # Use the lubridate
...
Ex4 <- Ex3 %>% 
  filter(depth < 0.25) %>% 
  summarize(mean_surface = mean(temperature_C)) # mean_surface =라고 한 이유는 이름 지정해주려고
  
Ex4_otherway <- Ex3 %>% 
  filter(depth == 0) %>% 
  drop_na(temperature_C) %>%  # filter our missing observation
  summarize(mean(temperature_C))
...
Ex5 <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck,
                    names_to = "Irradiance_Type",
                    values_to = "Irradiance_Value")
Ex5_otherway <- NTL.phys.data %>% 
  pivot_longer(
...
#Below is a practice
#Ex6 <- pivot_wider(NTL.phys.data, temperature_C:depth,
                    names_from = "Temperature C",
                    values_from = "Depth")

Ex6_otherway <- NTL.phys.data %>% 
  pivot_wider(
    names_from = depth,
