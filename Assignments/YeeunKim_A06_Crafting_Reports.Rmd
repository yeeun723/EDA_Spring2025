...
```{r echo=FALSE, fig.align='left', fig.cap="2018 Monthly Ozone Levels", fig.height=3, fig.width=7, warning=FALSE, out.height="45%", out.width="100%", results='hide'}
# create boxplots of the distribution of Ozone levels by month using, one for only records collected in 2018 and one for records in 2019.
library(ggplot2)
library(dplyr)

# Filter only the 2018 data
epa_2018 <- epa_data %>% 
  filter(Year==2018)

# Make a boxplot
epa_2018_box <- ggplot(epa_2018, aes(x=factor(Month), y=Ozone)) +
  geom_boxplot() +
  ggtitle("Monthly Ozone Concentrations (2018)") +
  labs(x="Month", y="Concentration ($\\mu g/m^3$)")
...
# Make a boxplot
epa_2019_box <- ggplot(epa_2019, aes(x=factor(Month), y=Ozone)) +
  geom_boxplot() +
...
## Git Repository
<https://github.com/yeeun723/EDA_Spring2025.git>
